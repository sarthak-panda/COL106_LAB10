...
bool search(Node* v, int x) {
    if (v == nullptr) return false;       // external node: not found

    int m = (int)v->keys.size();
    for (int i = 0; i < m; i++) {
        if (x == v->keys[i]) return true;           // found at this node
        if (x < v->keys[i]) {
            if (v->isLeaf) return false;            // no child to descend into
            return search(v->children[i], x);       // descend before keys[i]
        }
    }
    if (v->isLeaf) return false;                    // x > all keys, no children
    return search(v->children[m], x);              // x > all keys: go rightmost
}
